...
?>
  ...
  <tr>
	...
	<tr>
		<td colspan="2" valign="middle" height="22" class="cart" style="text-align: right">Discount (<?php echo @$cart['discount']['info']?>): </td>
	...
	<tr>
		<td colspan="3" style="text-align: right">	
			<form action="https://www.paypal.com/cgi-bin/webscr" method="post" accept-charset="utf-8">
				...
				<input type="hidden" name="custom" value="<?php echo $session->read('affiliate_code')?>">			

				
				<? $i = 1; foreach ($cart['items'] as $item) { ?>
					<input type="hidden" name="item_name_<?=$i?>" value="<?php echo $item['Product']['product_name']?>" />
					<input type="hidden" name="item_number_<?=$i?>" value="<?php echo $item['Product']['id']?>|<?php echo $item['Product']['product_code']?>" />
					<input type="hidden" name="amount_<?=$i?>" value="<?php echo $item['Product']['product_price']?>" />
