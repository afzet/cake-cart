		...
		<td colspan="12" class="smaller">
			...
			<form method="get" action="">
				<select name="movies" onChange="goto_URL(this.form.movies)">
					...
					echo '<option value="/c/'.$id.'/'. urlencode(str_replace(array('%',', ','& ','- '),'',htmlspecialchars_decode($category['Category']['name']))) .'.html">All</option>';
					
					foreach($cats as $cat) 
					{
						$cat_name = urlencode(str_replace(array('%',', ','& ','- '),'',htmlspecialchars_decode($cat['Category']['name'])));
						echo '<option value="/c/'.$cat['Category']['id'].'/'.$cat_name.'.html">
						'.$cat['Category']['name'].'
						</option>';
					}
					?>
