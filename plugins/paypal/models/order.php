<?php
class Order extends PaypalAppModel {
    /**
    ...
    function isValid($data){
      if(!empty($data)){
        App::import(array('type' => 'File', 'name' => 'Paypal.PaypalSource', 'file' => 'models'.DS.'datasources'.DS.'paypal_source.php'));
    ...
    function email($options = array()){
      ...
      $defaults = array(
        ...
        'sendAs' => 'html',
        'to' => $this->data['Order']['payer_email'],
        'from' => $this->data['Order']['business'],
        'cc' => array(),
        'bcc' => array(),
        'layout' => 'default',
        'template' => null,
    ...
    function buildAssociationsFromIPN($post){
      ...
      if(isset($post['num_cart_items']) && $post['num_cart_items'] > 1){
        ...
        for($i=1;$i<=$post['num_cart_items'];$i++){
          $retval['Item'][$i]['item_name'] = $post["item_name$i"];
          $retval['Item'][$i]['item_number'] = $post["item_number$i"];
          $retval['Item'][$i]['item_number'] = $post["item_number$i"];
          $retval['Item'][$i]['quantity'] = $post["quantity$i"];
          $retval['Item'][$i]['mc_shipping'] = $post["mc_shipping$i"];
          $retval['Item'][$i]['mc_handling'] = $post["mc_handling$i"];
          $retval['Item'][$i]['mc_gross'] = $post["mc_gross_$i"];
          $retval['Item'][$i]['tax'] = $post["tax$i"];
        }
      }
      return $retval;
    }
